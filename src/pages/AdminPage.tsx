...
function AdminPanel() {
  // Starts from the bundled defaults (renders immediately, same
  // fallback-first pattern as useSiteContent) then hot-swaps to the DB
  // values once loadFriendlyConfigFromDB() resolves below — no
  // localStorage draft anymore, DB is the sole persistence layer.
  const [config, dispatch] = useReducer(configDraftReducer, initialConfig)
  const [dbLoadStatus, setDbLoadStatus] = useState<'loading' | 'loaded' | 'error'>('loading')
    ...
    loadFriendlyConfigFromDB()
      .then(({ config: dbPatch }) => {
        ...
        if (Object.keys(dbPatch).length > 0) {
          dispatch({ type: 'replaceConfig', config: { ...initialConfig, ...dbPatch } as PortfolioConfig })
        }
        setDbLoadStatus('loaded')
      })
      .catch(() => {
        if (!cancelled) setDbLoadStatus('error')
      })
    return () => {
      cancelled = true
    }
  ...
  const handleSaveDraft = useCallback(() => {
    void persistFriendlyConfig('draft')
  }, [persistFriendlyConfig])

  const handlePublish = useCallback(() => {
    void persistFriendlyConfig('published')
  }, [persistFriendlyConfig])
  ...
  return (
    ...
    >
      ...
      {!CMS_TABS.includes(tab) && dbLoadStatus === 'error' && (
        <div
          className="mb-6 rounded-[var(--radius-base)] border border-amber-600 bg-amber-900/30 p-4 text-sm text-amber-200"
          role="alert"
        >
          Could not load saved content from the database — showing bundled defaults. Editing is still safe; Save Draft/Publish will retry.
        </div>
      )}
